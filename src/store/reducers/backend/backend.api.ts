...
export const backendApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: appConfig.api.url + appConfig.api.api,
  ...
  endpoints: (build) => ({
    ...
    smsVerificationCode: build.mutation<
      boolean,
      Pick<LoginMutationType, "data">
    >({
      query: (body) => ({ url: `/authorization/code`, method: "POST", body }),
    ...
    getUserInfo: build.query<
      GetCustomerInfoQueryType | GetCourierInfoQueryType,
      void
    >({
      query: () => ({ url: "/user/info" }),
      async onQueryStarted(arg, { dispatch, queryFulfilled }) {
    ...
    updateUserSettings: build.mutation<
      boolean,
      UpdateCustomerSettingsMutationType | UpdateCourierSettingsMutationType
    >({
      ...
      async onQueryStarted(update, { dispatch, queryFulfilled }) {
        try {
          ...
          notification.success({
            message: "Your settings updated successful!",
          });
          batch(() => {
